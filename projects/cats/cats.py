...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    def dif(argu):
        # 这里要注意顺序
        return diff_function(typed_word,argu,limit)
        
    if typed_word in word_list:
        return typed_word
    ans = min(word_list,key = dif)
    if dif(ans) > limit:
        return typed_word
    return ans
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 9
    for j in range(0,len(timestamps_per_player)):
        timestamps_per_player[j] = [timestamps_per_player[j][i] - timestamps_per_player[j][i-1] for i in range(1,len(timestamps_per_player[j]))]
...
def fastest_words(match):
    ...
    "*** YOUR CODE HERE ***"
    # 把时间（二维列表）搞出来
    ti = get_all_times(match)
    # 把项目（一位列表）搞出来
    wo = get_all_words(match)
    # 建一个二维列表，最后返回
    total_p = len(ti)
    fastest =[[] for i in range(total_p)]
    for i in word_indices:
        min = 0
        for j in player_indices:
            if ti[j][i] < ti[min][i]:
                min = j
        fastest[min].append(wo[i])
    # END PROBLEM 10
    return fastest
